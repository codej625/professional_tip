...
```zsh
# 호스트 PC는 VM의 IP 주소에 해당하는 MAC 주소를 기억하고 있다.
# IP 접속이 안 되면 IP를 삭제해보자.

# CMD (windows)
arp -d 192.168.0.59 (예시)
arp -d 192.168.0.201 (예시)

# zsh (linux & macOS)
sudo arp -d 192.168.0.59 (예시)
sudo arp -d 192.168.0.201 (예시)

(성공 메시지가 뜨지 않아도 내부적으로 캐시가 삭제된다.)
```
